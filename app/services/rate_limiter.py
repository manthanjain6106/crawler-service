"""
Rate limiting service for the crawler microservice.
Manages per-domain rate limiting with sliding window approach.
"""
...
class RateLimitService:
    ...
    def __init__(self):
        ...
        self.enabled = self.settings.enable_per_domain_rate_limiting
        self.default_limit = self.settings.default_domain_rate_limit
        self.window_size = self.settings.domain_rate_limit_window
        self.domain_limits = self.settings.domain_specific_limits.copy()
        ...
        self.logger.info(f"RateLimitService initialized - Enabled: {self.enabled}, "
    ...
    def _get_domain(self, url: str) -> str:
        ...
        try:
            ...
            self.logger.warning(f"Failed to parse domain from URL {url}: {e}")
            return "unknown"
    ...
    async def can_make_request(self, url: str) -> bool:
        ...
        async with self.locks[domain]:
            ...
            if not can_request:
                self.logger.debug(f"Rate limit exceeded for domain {domain}: "
    ...
    async def record_request(self, url: str) -> None:
        ...
        async with self.locks[domain]:
            ...
            self.logger.debug(f"Recorded request for domain {domain}: "
    ...
    async def wait_if_needed(self, url: str) -> None:
        ...
        if wait_time > 0:
            self.logger.info(f"Rate limiting: waiting {wait_time:.2f}s before request to {url}")
    ...
    def set_domain_limit(self, domain: str, limit: int) -> None:
        ...
        self.logger.info(f"Set custom rate limit for {domain}: {limit} requests per {self.window_size}s")
    ...
    def remove_domain_limit(self, domain: str) -> None:
        ...
        if domain in self.domain_limits:
            ...
            self.logger.info(f"Removed custom rate limit for {domain}, using default: {self.default_limit}")
    
    async def shutdown(self) -> None:
        """Shutdown the rate limiter service."""
        self.logger.info("Rate limiter service shutdown complete")
